...
import { Eye, Heart, Share2, Bed, Bath, Car, Ruler, Filter, Grid, List } from 'lucide-react'
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex items-center gap-4">
              <span className="text-gray-600">Showing {houses.length} results</span>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              >
                <div className={`relative overflow-hidden ${viewMode === 'list' ? 'w-1/3' : ''}`}>
                  ...
                  <div className="absolute top-3 right-3 bg-white/95 backdrop-blur-sm rounded-lg px-3 py-2">
                    <div className="text-emerald-600 font-bold text-lg">{house.price}</div>
                    {house.originalPrice && (
                      <div className="text-gray-500 text-sm line-through">{house.originalPrice}</div>
                    )}
                  </div>
                ...
                <div className={`p-6 ${viewMode === 'list' ? 'flex-1' : ''}`}>
                  <div className="mb-3">
                    ...
                    <p className="text-sm text-gray-500 mb-2">Plan #{house.planNumber}</p>
                    {viewMode === 'list' && (
                  ...
                  <div className="mb-4">
                    <div className="flex flex-wrap gap-1">
                      {house.greenFeatures.slice(0, 2).map((feature, index) => (
                        <span 
                          key={index}
                      ...
                      {house.greenFeatures.length > 2 && (
                        <span className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-full">
                          +{house.greenFeatures.length - 2} more
                        </span>
                      )}
                    </div>
                  </div>
                  
                  <Link href={`/house/${house.id}`} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-3 px-4 rounded-lg transition-colors duration-300 block text-center">
                    View Details
                  </Link>
                </div>
              </div>
          ...
          <div className="flex justify-center mt-12">
            <nav className="flex items-center space-x-2">
              ...
              <button className="px-3 py-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg">2</button>
              <button className="px-3 py-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg">3</button>
              <button className="px-3 py-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg">
                Next
              </button>
            </nav>
